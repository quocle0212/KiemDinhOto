...
const DetailOrder = ({ intl }) => {
  ...
  // Tách theo các dòng mới và loại bỏ dòng trống
  return noteString
    .split(/\n+/) // tách theo một hoặc nhiều dòng mới
    ...
    return (
      <div>
        ...
        <div className='mt-2'>
          ...
          <div className='font-weight-bolder mt-1'>Ghi chú : {
            splitNotes(order?.note).length > 0 ? (
              <ul>
                {splitNotes(order?.note).map((note, index) => (
