...
const Order = ({ intl, startDay, endDay, stationType, scheduleType, orderStatus, hiddenOrderStatus }) => {
  ...
  const columns = [
    {
      ...
      cell: (row, index) => {
        ...
        return (
          <div className="order-id" onClick={() => history.push(`/pages/detail-order/${row.orderId}`)}>
            {row?.orderId}
          </div>
        )
      }
