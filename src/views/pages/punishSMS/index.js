...
import { PAYMENT_METHOD_TYPE } from '../station/paymentMethod'

const PunishSMS = ({ intl, startDay, endDay, stationType, scheduleType, orderStatus, hiddenOrderStatus }) => {
  ...
  const DefaultFilter = {
    filter: {
      ...
      orderType: 2, // tin nhắn phạt nguội
  ...
  const columns = [
    {
      ...
      cell: (row, index) => {
        ...
        return (
          <div
            className="order-id"
            // onClick={()=>history.push(`/pages/detail-punishsms/${row.orderId}`)}
      ...
      cell: (row) => {
        ...
        return (
          <div>
            ...
            <div className="order-text">
              Trạng thái : <span style={{ color: paramsOrder.color, fontWeight: 'bold' }}>{intl.formatMessage({ id: paramsOrder?.label })}</span>
            </div>
      ...
      cell: (row) => (
        <span>
          ...
          <span
            ...
            onClick={(e) => {
              e.preventDefault()
              // history.push(`/pages/detail-punishsms/${row.orderId}`)
              history.push(`/pages/detail-order/${row.orderId}`)
            }}>
            {<Edit size={16} />}
          </span>
  ...
  const createRowData = (row) => {
    return {
      ...
      EMAIL: row?.email,
      'ID ĐƠN HÀNG': row?.orderId,
      'MÃ ĐƠN HÀNG': row?.orderCode,
      'TÊN ĐƠN HÀNG': row?.orderName || Object.values(ORDER_TYPE).find((el) => String(el.value) === String(row?.orderType))?.label,
      'THỜI GIAN MUA HÀNG': row?.createdAt ? convertTimeDate(row?.createdAt) : '',
      'TỔNG TIỀN SẢN PHẨM CỦA ĐƠN HÀNG': row?.subTotalAmount,
      'TỔNG TIỀN THUẾ': row?.taxAmount,
      'PHẦN TRĂM THUẾ': (row?.orderItems || [])
        .map((item, index) => `SP ${index + 1}: ${item?.productName || ''} - ${item?.productTax * 100 || 0}%`)
        .join('\n'),
      'TÊN THUẾ': (row?.orderItems || [])
        .map((item, index) => `SP ${index + 1}: ${item?.productName || ''} - ${item?.productTaxName || ''}`)
        .join('\n'),
      'SỐ TIỀN PHỤ PHÍ  ': row?.extraFee,
      'PHẦN TRĂM PHỤ PHÍ': `${row?.row?.extraFeePercent || 0}%`,
      'TÊN PHỤ PHÍ': row?.extraName,
      'SỐ TIỀN GIẢM GIÁ': row?.discountAmount,
      'PHẦN TRĂM GIẢM GIÁ': `${row?.promotionData?.promotionPercentage * 100 || 0}%`,
      'MÃ GIẢM GIÁ ÁP DỤNG': row?.promotionData?.promotionName,
      'TỔNG TIỀN PHẢI THU KHÁCH HÀNG': row?.totalAmount,
      'TRẠNG THÁI XỬ LÝ ĐƠN HÀNG': intl.formatMessage({ id: stations_location.find((el) => el.value === row?.orderStatus)?.label }),
      'THỜI GIAN CẬP NHẬT ĐƠN HÀNG': row?.updatedAt ? convertTimeDate(row?.updatedAt) : '',
      'TRẠNG THÁI THANH TOÁN ĐƠN HÀNG': ORDER_PAYMENT_STATUS.find((el) => el.value === row?.paymentStatus)?.label,
      'THỜI GIAN THANH TOÁN ĐƠN HÀNG': row?.updatedAt ? convertTimeDate(row?.orderPaymentTime) : '',
      'PHƯƠNG THỨC THANH TOÁN': (
        Object.keys(PAYMENT_METHOD_TYPE).find((key) => PAYMENT_METHOD_TYPE[key] === row?.paymentMethodData?.paymentMethodType) || ''
      )?.replaceAll('_', ' '),
      'MÃ LỆNH THANH TOÁN': row?.orderRefId,
      'NƠI THANH TOÁN': row?.paymentMethodData?.paymentMethodReferName,
      'THÔNG TIN SHIP HÀNG': `
        Địa chỉ: ${row?.orderShippingData?.recipientAddress || ''}.\n
        Người nhận: ${row?.orderShippingData?.recipientName || row?.orderShippingData?.recipientPhone || ''}.\n
        Thời gian giao hàng: ${row?.orderShippingData?.deliveryStartTime ? convertTimeDate(row?.orderShippingData.deliveryStartTime) : ''}.\n
        Thời gian nhận hàng dự kiến: ${
          row?.orderShippingData?.estimatedReceiptTime ? convertTimeDate(row?.orderShippingData.estimatedReceiptTime) : ''
        }.
      `,
      'THÔNG TIN XUẤT HÓA ĐƠN': `
        Địa chỉ: ${row?.customerReceiptData?.customerReceiptAddress || ''}.\n
        Người nhận: ${row?.customerReceiptData?.customerReceiptName || row?.customerReceiptData?.customerReceiptPhone || ''}.\n
        Mã định danh: ${row?.customerReceiptData?.taxCode || ''}.\n
        Số tiền: ${row?.customerReceiptData?.customerReceiptAmount || 0}.
      `
    }
  }
        ...
        <Row className="mx-0 mt-1">
          ...
          <Col sm="4" xs="12" lg="3" className="mb-1">
            <LoadingDialogExportFile
              title={`Xuất file Danh sách đơn phạt nguội`}
              createRowData={createRowData}
              filter={paramsFilter}
              linkApi={'Order/find'}
              nameFile={`Danh sách đơn phạt nguội`}
            />
          </Col>
        </Row>
...
export default injectIntl(PunishSMS)
