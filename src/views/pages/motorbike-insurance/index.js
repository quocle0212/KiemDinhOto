...
const MotorbikeInsurance = ({ intl, startDay, endDay, stationType, scheduleType, orderStatus, hiddenOrderStatus }) => {
  ...
  const columns = [
    {
      ...
      cell: (row, index) => {
        ...
        return (
          <div
            className="order-id"
            // onClick={()=>history.push(`/pages/detail-motorbikeInsurance/${row.orderId}`)}
      ...
      cell: (row) => (
        <span>
          ...
          <span
            ...
            onClick={(e) => {
              e.preventDefault()
              // history.push(`/pages/detail-motorbikeInsurance/${row.orderId}`)
              history.push(`/pages/detail-order/${row.orderId}`)
            }}>
            {<Edit size={16} />}
          </span>
        ...
        <Row className="mx-0 mt-1">
          ...
          <Col sm="4" xs="12" lg="3" className="mb-1">
            <LoadingDialogExportFile
              title={`Xuất file Danh sách bảo hiểm xe máy`}
              createRowData={createRowData}
              filter={paramsFilter}
              linkApi={'Order/find'}
              nameFile={`Danh sách bảo hiểm xe máy`}
            />
          </Col>
        </Row>
...
export default injectIntl(MotorbikeInsurance)
