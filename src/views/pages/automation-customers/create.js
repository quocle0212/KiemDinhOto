...
const CreateAutomationCustomer = ({ intl }) => {
  ...
  const [isSelectAnotherTime, setIsSelectAnotherTime] = useState(false)
  const [haveSelectTimeSending, setHaveSelectTimeSending] = useState(false)

  const [enableNotiByAPNSData, setEnableNotiByAPNSData] = useState({ enable: false, data: [] })
  const [enableNotiBySmsCSKHData, setEnableNotiBySmsCSKHData] = useState({ enable: false, data: [] })
  const [enableNotiByZaloCSKHData, setEnableNotiByZaloCSKHData] = useState({ enable: false, data: [] })
  const [enableNotiBySMSRetryData, setEnableNotiBySMSRetryData] = useState({ enable: false, data: [] })
  const [enableNotiByAutoCallData, setEnableNotiByAutoCallData] = useState({ enable: false, data: [] })
  ...
  const handleChangeStation = (selected) => {
    ...
    setSettings((prev) => ({
      ...prev,
      stationsId: selected?.value || null
    }))
  }
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = { ...settings, configTemplateName: templateName.trim(), stationsId: settings?.stationsId || null, enableConfig: 1 }

    const isValid = validateForm(payload)
    if (!isValid) return

    try {
      const res = await StationMessageConfigService.createStationMessageConfig({ data: payload }, token)

      if (res?.statusCode === 200) {
        toast.success('Tạo cấu hình thành công')
        history.push('/pages/automatic-takecare-customer')
      } else {
        toast.error(res?.error || 'Tạo cấu hình thất bại')
      }
    } catch (err) {
            ...
            <FormGroup>
              ...
              <div className="d-flex flex-column" style={{ gap: '1rem' }}>
                ...
                <SelectAntd
                  className="mt-1"
                  placeholder={intl.formatMessage({ id: 'chooseMessageTemplate' })}
                  onChange={(option) => handleChangeTemplate(option, 'messageTemplateAutoCall')}
                  style={{
                    width: '100%',
                    height: '38px',
                    outline: 'none',
...
export default memo(injectIntl(CreateAutomationCustomer))
