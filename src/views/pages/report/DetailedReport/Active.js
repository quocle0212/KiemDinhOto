...
const Active = ({ intl, listStation }) => {

  const newArr = listStation?.map(item =>{
    return item.stationStatus === 1 ? 100 : 0
  })

  const percent = newArr.map(item =>{
...
export default injectIntl(memo(Active))
